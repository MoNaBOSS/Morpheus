...
function buildClawXUvConfigToml(): string {
  return [
    '# This file is managed by Morpheus.',
    '# It lets uv use Morpheus-selected mirrors even when host exec filters UV_INDEX_URL.',
...
function ensureClawXUvConfigFile(): string | null {
  ...
  try {
    ...
    logger.warn('Failed to write Morpheus uv config file:', err);
    return null;
  }
}
