...
<p align="center">
  <strong>デスクトップ向け AI 実行プラットフォーム／AI システムビルダー</strong>
</p>

<p align="center">
  <a href="#機能">機能</a> •
  <a href="#なぜmorpheusなのか">なぜMorpheusなのか</a> •
  <a href="#はじめに">はじめに</a> •
...
**Morpheus** は、ユーザーの目的を型付き実行プランへ変換し、信頼境界を評価し、決定論的な機能を実行します。進行状況、成果物、追記専用履歴を通じて、作業全体を常に観測できます。チャットはプラットフォームへの一つの入口であり、製品そのものではありません。

[OpenClaw](https://github.com/OpenClaw) は組み込みのエージェント／チャットランタイムとして維持されます。Morpheus はコマンドセンター、実行プラン、権限、機能、ワークフロー、スケジュール、成果物、監査履歴を所有します。モデルプロバイダーは交換可能な推論バックエンドであり、無制限の OS 権限を直接受け取ることはありません。
...
AIエージェントの構築にコマンドラインの習得は不要であるべきです。Morpheusはシンプルな哲学のもとに設計されました：**強力な技術には、あなたの時間を尊重するインターフェースがふさわしい。**
...
Morpheusは公式の**OpenClaw**コアを直接ベースに構築されています。別途インストールを必要とせず、アプリケーション内にランタイムを組み込むことで、シームレスな「バッテリー同梱」体験を提供します。
...
`@agent` で別のエージェントを選ぶと、Morpheus はデフォルトエージェントを経由せず、そのエージェント自身の会話コンテキストへ直接切り替えます。各エージェントのワークスペースは既定で分離されていますが、より強い実行時分離は OpenClaw の sandbox 設定に依存します。
...
Chat の右パネルにあるワークスペースとプレビューの各タブでは、Markdown、`.docx`、`.pptx` ファイルを読み取り専用でプレビューできます。Markdown ファイルのプレビューでは、同じシンタックスハイライト、ソフトラップ、コピー操作付きのフェンスコード、CJK 対応の解析、KaTeX 数式を静的レンダリングモードでサポートします。プレビューのヘッダーから選択中のファイルを Morpheus の表示領域全体に拡大でき、同じボタンまたは Esc で右パネルへ戻れます。従来形式の `.doc` と `.ppt` はアプリ内ではプレビューせず、引き続き OS 経由で開きます。DOCX のページ区切りは Microsoft Word と異なる場合があり、PPTX プレビューではアニメーション、画面切り替え、メディア再生をサポートしません。20 MB を超える Office ファイルはアプリ内でプレビューされません。

### ローカル HTML プレビュー
Chat の右パネルにはワークスペース、プレビュー、変更だけがあり、汎用ウェブブラウザ、ホーム画面、アドレスバーはありません。許可済みのローカル `.html` / `.htm` 添付ファイル、ファイルアクティビティ、ワークスペースファイルは既定でプレビューに開きます。ファイル操作では Morpheus 内蔵プレビューまたはシステムアプリを選択でき、プレビューのヘッダーから現在の HTML ファイルをシステムブラウザで開くこともできます。

すべてのリンクはクリックできません。Morpheus が描画するリンクは通常のテキストとして表示され、HTML プレビュー内のリンクからもリンク装飾とポインター操作が除去されます。フォーム、スクリプトによる移動、リダイレクト、ページ内移動、ポップアップ、ダウンロード、ネットワーク要求、デバイス権限もブロックされます。自己完結したローカル HTML は表示できますが、選択中の文書から移動することはできません。

モデルプロバイダーを設定すると、Morpheus Objective Core は承認済み Morpheus ワークスペース内に制限された自己完結型ビジネスサイトを作成し、実在するレスポンシブファイルと分析対応設定を検証し、確認済みの入口を Command Center の検査可能な成果物として表示し、Morpheus 管理のフォローアップ通知を作成できます。対応するデプロイ機能が実際に成功しない限り、公開デプロイ済みとは表示しません。
...
Morpheus には Tencent 公式の個人 WeChat チャンネルプラグインも同梱されており、Channels ページからアプリ内 QR フローで直接 WeChat を連携できます。
...
Morpheus はドキュメント処理スキル（`pdf`、`xlsx`、`docx`、`pptx`）もフル内容で同梱し、起動時に管理スキルディレクトリ（既定 `~/.openclaw/skills`）へ自動配備し、初回インストール時に既定で有効化します。
...
互換ゲートウェイで `/models` が認証以外の理由で使えない場合、Morpheus は API キー検証時に設定済みモデルを使った軽量な `/chat/completions` または `/responses` プローブへ自動フォールバックします。

### 🌙 アダプティブテーマ
ライトモード、ダークモード、またはシステム同期テーマ。Morpheusはあなたの好みに自動的に適応します。
...
Morpheus は **Signal OS** として起動します。独自のシグナル・アイデンティティ、
シネマティックな有効化、コンパクトな製品ナビゲーション、
**Today / Mission / Context** コマンドセンターを備えた、落ち着いた実行中心の
オペレーティング・プレゼンスです。Chat は引き続き利用できますが、ホーム体験を
定義するものではありません。

1.0.3 候補版はニューラル音声の失敗を明示し、保存失敗時には以前の音声設定を保持します。
CI パッケージ前に全単体テストと主要 Electron テストを実行し、リリースは草稿として作成します。
Morpheus の署名設定が必要です。候補版は一般公開の承認を意味しません。

1.0.2 では計画リクエストと出力に上限を設け、内容を含まない使用量を記録し、
メインプロセスで音声リクエストをキャンセルします。音声は短く、全文はタスクに残ります。
明示的に有効化したバックグラウンド音声監視は描画フレームに依存しません。
これは金額保証ではなく、独立した OpenClaw チャットは対象外です。料金画面は追加しません。

1.0.1 では起動後に既存ユーザーを迎え、「ワークスペースを開く」または
「トレイで待機」を選べます。コマンドセンターの「コンパニオン」で再表示できます。
トレイへの移動はマイクを有効にしません。有効化の最終画面は選択を待ちます。
音声停止は遅れて届く音声の再生も防ぎ、動きはシステムの低減設定に従います。

Morpheus は `/` の**コマンドセンター**を既定画面とし、`Ctrl+Shift+Space` でどこからでも**クイックコマンド**を開けます。**質問 / 自動 / 実行**は音声、クイックコマンド、コマンドセンター、Chat で共通の制御です。既定の自動モードでは Main が質問を OpenClaw に、明確な結果要求を同じ Objective Core に送り、不明確な指示には一度だけ確認します。対象解決と実行権限は Main が保持します。

コンパニオン／Mission 基盤により、ランタイムは継続的な操作体験へ拡張されます。

- 一度だけ表示されスキップ可能なシネマティック有効化は実際の可用性だけを表示し、呼び名、人格、操作モード、自律性、Windows 起動、開示済みウェイクフレーズ、音声結果、チェックインを設定します。
- トレイとグローバルショートカットは集中型コンパニオン画面を呼び出し、閉じる／展開する際に元のウィンドウ状態を正確に復元します。
- 受理された目標はすべて永続 **Mission** となり、ルート、実行系譜、状態、結果、エラー、実際の成果物を再起動後も保持します。
- **Projects & Context** は目標を論理的な信頼済みワークスペースと、確認・管理可能なメモリに結び付けます。機密またはローカル専用メモリはプロバイダーの計画コンテキストへ入りません。
- 既知の目標はプロバイダー選択前に登録済み機能へ直接ルーティングされ、広い目標は同じプロバイダー中立 Objective Core を通ります。音声、Quick Command、Command Center、workflow、schedule、Chat 実行は別々の自動化経路を作りません。
- オプトインのアンビエント音声は明示的な開示後だけ有効になり、正確なウェイクフレーズと可視状態を保ったまま同じ Objective Core に入ります。
- **Today** は実在する Mission、Goal、schedule、反復作業から注意項目を作り、dismiss／snooze と制限付き通知状態を永続化します。
- 長期 **Goals** は測定可能な milestone と正確な実行コンテキストを保持し、**Systems** はレビュー済み workflow または適格 Mission を実テスト済みの再利用可能作業へ変換しますが、permission grant は作りません。
- 初回コンパニオン有効化は実際の状態が整ったときだけ **MORPHEUS IS READY** を表示し、Command Center、グローバル Quick Command、音声入口を案内します。
- 有効化画面と Command Center は、互換プロトコルと資格情報を持つアカウントだけを Objective Core 準備完了として表示します。未設定時は Models へ直接案内し、音声が不明瞭な場合は自然に一度だけ聞き直し、設定エラーでは同じ設定入口を提示します。
...
- **プラン単位の信頼評価** — Strict、Balanced、Autonomous（新規プライベートアルファの既定）はプラン全体を評価し、本当に新しいスコープだけを一度確認します。正確なセッション／永続グラントを再利用し、critical 境界は回避できません。
...
- **Signal OS ビジュアルシステム** — ダークを既定とし、1280×800 に収まる密度。Matrix の影響は実際のライブ／検証済み状態に限定し、独自の Morpheus シグナル文法、シネマティックな初回有効化、実行中心の Mission ステージ、コンパクトな Presence/Invoke 画面を備えます。俳優の肖像は使用しません。
...
Morpheusを初めて起動すると、**セットアップウィザード**が以下の手順をガイドします：
...
Morpheusには、Electron、OpenClaw Gateway、またはTelegramなどのチャネルがローカルプロキシクライアントを介してインターネットにアクセスする必要がある環境向けに、組み込みのプロキシ設定が含まれています。
...
- 高度なプロキシフィールドが空の場合、Morpheusは`プロキシサーバー`にフォールバックします。
- プロキシ設定を保存すると、Electronのネットワーク設定が即座に再適用され、ゲートウェイが自動的に再起動されます。
- MorpheusはTelegramが有効な場合、プロキシをOpenClawのTelegramチャネル設定にも同期します。
- Morpheusのプロキシが無効な状態では、Gatewayの通常再起動時に既存のTelegramチャネルプロキシ設定を保持します。
...
- 完全な ACP リプレイからは記録済みのファイルアクティビティを復元できます。リプレイが不完全な場合、Morpheus はフォールバック推論で欠落したアクティビティを補いません。
...
- Morpheus は Electron アプリのため、**1つのアプリインスタンスでも複数プロセス（main/renderer/zygote/utility）が表示される**のが正常です。
...
- ウィンドウの閉じるボタン（`X`）は既定でトレイへ最小化する動作で、完全終了ではありません。完全終了する場合はトレイメニューの **Quit Morpheus** を使用してください。
...
Morpheus は Chromium のハードウェアアクセラレーションを既定で有効なままにし、長い文書、スクロール、レイアウトアニメーションで GPU コンポジットとラスタライズを利用します。グラフィックスドライバーに問題があるマシンでは、Chromium 標準の `--disable-gpu` コマンドラインスイッチをトラブルシューティング用のフォールバックとして利用できます。
...
コミュニティからのコントリビューションを歓迎します！バグ修正、新機能、ドキュメントの改善、翻訳など、あらゆる貢献がMorpheusをより良くするのに役立ちます。
...
Morpheusは優れたオープンソースプロジェクトの上に構築されています：
...
Morpheus パートナープログラムを開始します。特に、カスタム AI エージェントや自動化ニーズを持つより多くの顧客に Morpheus を紹介してくださるパートナーを募集しています。

パートナーの皆さまには、見込みユーザーや案件との接点づくりを担っていただき、Morpheus チームは技術サポート、カスタマイズ、統合を全面的に提供します。
...
Morpheusは[MITライセンス](LICENSE)の下でリリースされています。本ソフトウェアの使用、変更、配布は自由に行えます。
