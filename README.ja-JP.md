...
### 🟢 Morpheus コマンドセンター（Concept Build 0.1）

Morpheus 製品面の最初のマイルストーンです。サイドバーの **ダッシュボード** から、エージェントランタイム
ではなく Electron メインプロセスで実行されるネイティブ操作のコマンドセンターを開けます。

- **ブートシーケンス** — 実際のシグナル（設定のハイドレーション、ホストブリッジの往復、ゲートウェイの
  ステータス）で進行する、マトリックス風の起動オーバーレイ。`Esc` またはクリックでスキップでき、常に
  一定時間内に自動で消えます。
- **ネイティブ操作** — 承認済みアプリケーションの起動、メインプロセス管理下のフォルダへのテキスト
  ファイル作成、プライバシーに配慮したシステム情報の取得。これらは拡張可能な操作レジストリの最初の
  エントリであり、固定のセットではありません。
- **明示的な確認** — 実行前に、メインプロセスが実際に*解決した*対象（実行ファイルまたはファイルの絶対
  パス）を必ず表示します。拒否した場合は一切の処理を行いません。
- **実行タイムライン** — メインプロセスが報告する実際のフェーズのみを表示します。シミュレーションは
  ありません。
- **監査ログ** — ユーザーデータフォルダ配下の追記専用 JSONL 記録。各フェーズは画面に届く前に書き込まれ、
  ファイル内容は保存されません（サイズとダイジェストのみ）。

操作ポリシーはアプリケーションに組み込まれており、画面から変更することはできません。レンダラーが送る
のは論理的な操作 ID と検証済みパラメーターだけで、実行ファイルのパス・コマンドライン・環境変数・任意の
ファイルパスを送ることはありません。詳細は
[`harness/reference/morpheus-execution-architecture.md`](harness/reference/morpheus-execution-architecture.md)
を参照してください。

現在 Windows で 3 つの機能を提供しています。レジストリはプラットフォームを認識するため、macOS と Linux
の実装はランタイム・コントラクト・画面を変更せずにモジュールの追加だけで対応できます。
