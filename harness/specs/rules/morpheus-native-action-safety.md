...
Executable resolution requires an exact registry key or compiled-in template match without normalization or case folding, a base directory taken from a trusted process environment value rather than from any payload, a link status check confirming a regular non-symbolic file, and a real path containment check proving the resolved target still lies inside the expected system directory. Native processes are launched with shell disabled, with fixed arguments or a single Main-canonicalized approved-workspace target, with window hiding on Windows, and with a bounded abort timer. URL opening accepts only validated http(s) URLs through Electron's external-browser bridge; custom protocols, file URLs, shell strings and command templates are not permitted.
...
Permission is evaluated against the complete plan and the active profile. Privacy-safe low-risk operations may run automatically; medium and high-risk operations ask only for a new trust boundary and may use exact session or persistent grants. Critical operations remain mandatory-confirmation. Execution is reachable only from the awaiting-permission phase when a boundary needs a decision, only for a live run identifier, and only once; the pending record is removed before execution so a repeated response cannot cause a second run. Unanswered requests expire and are recorded as denied. Concurrency and request rate are bounded.
