...
### 🟢 Командный центр Morpheus (Concept Build 0.1)

Первая веха продуктовой поверхности Morpheus. Пункт **Панель** в боковом меню открывает командный
центр для системных действий, которые выполняются в основном процессе Electron, а не через среду
агента.

- **Загрузочная последовательность** — заставка в стиле «Матрицы», которая продвигается по реальным
  сигналам (гидратация настроек, обращение к мосту узла, статус шлюза). Пропускается по `Esc` или
  щелчку и всегда закрывается сама в пределах ограниченного времени.
- **Системные действия** — запуск одобренного приложения, создание текстового файла в папке под
  контролем основного процесса, получение безопасных сведений о системе. Это первые записи
  расширяемого реестра действий, а не окончательный набор.
- **Явное подтверждение** — перед запуском всегда показывается цель, которую основной процесс
  действительно *разрешил* (абсолютный путь к программе или файлу). При отказе не выполняется ничего.
- **Хронология выполнения** — реальные фазы, о которых сообщает основной процесс. Ничего не
  имитируется.
- **Журнал аудита** — запись в формате JSONL только на добавление в папке пользовательских данных.
  Каждая фаза записывается до того, как попадёт в интерфейс, а содержимое файлов не сохраняется
  (только размер и дайджест).

Политика действий встроена в приложение и не может быть изменена из интерфейса. Отрисовщик передаёт
только логический идентификатор действия и проверенные параметры — никогда путь к программе, командную
строку, переменные окружения или произвольный путь в файловой системе. См.
[`harness/reference/morpheus-execution-architecture.md`](harness/reference/morpheus-execution-architecture.md).

Сейчас три возможности доступны в Windows; реестр учитывает платформу, поэтому реализации для macOS и
Linux добавляются новыми модулями без изменения среды выполнения, контракта или интерфейса.
