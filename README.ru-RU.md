...
<p align="center">
<strong>Платформа выполнения ИИ и конструктор ИИ-систем для рабочего стола</strong>
</p>

<p align="center">
<a href="#возможности">Возможности</a> •
<a href="#почему-morpheus">Почему Morpheus</a> •
...
**Morpheus** превращает цель пользователя в типизированный план выполнения, оценивает границы доверия, запускает детерминированные возможности и показывает реальный прогресс, артефакты и журнал истории. Чат — лишь один из интерфейсов платформы, а не сам продукт.

[OpenClaw](https://github.com/OpenClaw) остаётся встроенной средой агентов и чата. Morpheus владеет Командным центром, планами, разрешениями, возможностями, рабочими процессами, расписаниями, артефактами и аудитом. Провайдеры моделей взаимозаменяемы и не получают неограниченных полномочий ОС.
...
Создание AI-агентов не должно требовать владения командной строкой. Философия Morpheus проста: **мощные технологии заслуживают интерфейса, который уважает ваше время.**

| Проблема | Решение Morpheus |
|----------|---------------|
...
Morpheus построен непосредственно на официальном ядре **OpenClaw**. Вместо отдельной установки мы встраиваем среду выполнения в приложение для бесшовного опыта "всё включено".
...
При выборе другого агента через `@agent` Morpheus переключается непосредственно в контекст этого агента вместо ретрансляции через агента по умолчанию. Рабочие пространства агентов по умолчанию разделены, но более строгая изоляция зависит от настроек песочницы OpenClaw.
...
На правой панели Chat остаются только вкладки «Рабочая область», «Просмотр» и «Изменения»; универсального веб-браузера, домашней страницы и адресной строки больше нет. Разрешённые локальные вложения `.html` / `.htm`, файловые операции и файлы рабочей области по умолчанию открываются в «Просмотре». В действиях файла можно выбрать встроенный просмотр Morpheus или системное приложение, а кнопка в заголовке просмотра открывает текущий HTML-файл в системном браузере.

Все ссылки некликабельны. Ссылки, отображаемые Morpheus, выглядят как обычный текст; в HTML-просмотре также удаляются оформление ссылок и взаимодействие указателем. Формы, переходы из скриптов, перенаправления, переходы внутри страницы, всплывающие окна, загрузки, сетевые запросы и разрешения устройств блокируются. Самодостаточный локальный HTML отображается, но не может покинуть выбранный документ.

При настроенном провайдере Morpheus Objective Core также может создать ограниченный самодостаточный проект бизнес-сайта внутри одобренной рабочей области Morpheus, проверить реальные адаптивные файлы и конфигурацию для аналитики, показать проверенную точку входа как доступный для проверки артефакт Command Center и создать настоящее напоминание Morpheus. Публичное развёртывание не заявляется, пока поддерживаемая возможность развёртывания действительно не завершится успешно.
...
Для пользовательских идентификаторов учётных записей каналов Morpheus требует совместимый с OpenClaw канонический формат (`[a-z0-9_-]`, строчные буквы, максимум 64 символа, должен начинаться с буквы или цифры) для предотвращения ошибок маршрутизации.
Morpheus также включает официальный плагин личного WeChat от Tencent, позволяя подключить WeChat напрямую со страницы Каналы через встроенный QR-код.
...
Morpheus также предварительно упаковывает полные навыки обработки документов (`pdf`, `xlsx`, `docx`, `pptx`), автоматически развёртывает их в управляемый каталог навыков (по умолчанию `~/.openclaw/skills`) при запуске и включает по умолчанию при первой установке.
...
Когда совместимый шлюз отклоняет `/models` по причинам, не связанным с аутентификацией, Morpheus автоматически переключается на легковесный зонд `/chat/completions` или `/responses` при проверке API-ключа.

### 🌙 Адаптивные темы
Светлая тема, тёмная тема или синхронизация с системой. Morpheus автоматически адаптируется к вашим предпочтениям.
...
`Ctrl+Shift+Space`; **Voice Command** имеет отдельное глобальное сочетание клавиш; чат OpenClaw остаётся полностью доступным на `/chat`. **Спросить / Авто / Действовать** — единый переключатель для голоса, Quick Command, Command Center и Chat. В режиме Авто Main направляет вопросы в OpenClaw, запросы результата — в общий Objective Core, а для неясной команды просит одно уточнение.

Основа компаньона и Mission превращает этот runtime в постоянную рабочую среду:

- одноразовая, пропускаемая кинематографичная активация показывает только реальные сигналы и настраивает обращение, характер, режим, автономность, запуск с Windows, раскрытую голосовую активацию, озвучивание и инициативные вопросы;
- трей и глобальные сочетания вызывают сфокусированную поверхность компаньона и точно восстанавливают прежнее состояние окна после закрытия или разворачивания;
- каждая принятая цель становится долговечной **Mission** с маршрутом, историей запусков, статусом, результатами, ошибками и настоящими артефактами, сохраняемыми после перезапуска;
- **Projects & Context** связывает цели с логическим доверенным workspace и управляемой пользователем памятью; чувствительная и local-only память не передаётся в контекст планирования провайдера;
- известные цели сначала направляются прямо в зарегистрированные возможности, а широкие цели проходят через тот же нейтральный к провайдеру Objective Core. Голос, Quick Command, Command Center, workflow, schedule и выполнение из Chat не создают разрозненных путей автоматизации.
- Экран активации и Command Center показывают готовность Objective Core только для учётной записи с совместимым протоколом и настроенными данными доступа. При отсутствии настройки они ведут прямо в Models; при неясной речи Morpheus естественно просит повторить один раз, а при ошибке конфигурации предлагает тот же путь настройки.
...
При первом запуске Morpheus **Мастер настройки** проведёт вас через:
...
Morpheus включает встроенные настройки прокси для сред, где Electron, шлюз OpenClaw или каналы вроде Telegram должны выходить в интернет через локальный прокси-клиент.
...
- Если расширенные поля прокси пусты, Morpheus использует `Прокси-сервер`.
- Сохранение настроек прокси немедленно повторно применяет сеть Electron и автоматически перезапускает шлюз.
- Morpheus также синхронизирует прокси с конфигурацией канала Telegram в OpenClaw, когда Telegram включён.
- При перезапуске шлюза существующий прокси канала Telegram сохраняется, если прокси Morpheus отключен.
...
Morpheus использует **двухпроцессную архитектуру с унифицированным уровнем Host API**. Рендерер обращается к единой абстракции клиента, а Electron Main управляет выбором протокола и жизненным циклом процессов:

```
┌─────────────────────────────────────────────────────────────────┐
│                     Десктоп-приложение Morpheus                    │
│                                                                 │
│  ┌────────────────────────────────────────────────────────────┐ │
...
- Morpheus — это приложение Electron, поэтому **один экземпляр приложения обычно отображается как несколько процессов ОС** (main/renderer/zygote/utility). Это нормально.
...
- Нажатие кнопки закрытия окна (`X`) скрывает Morpheus в трей; это **не** полностью закрывает приложение. Используйте меню трея **Quit Morpheus** для полного завершения.
...
- использует временный каталог `userData` Morpheus
...
Мы приветствуем вклад сообщества! Исправления багов, новые функции, улучшения документации или переводы — каждый вклад делает Morpheus лучше.
...
Morpheus построен на плечах отличных проектов с открытым исходным кодом:
...
### Партнёрская программа Morpheus 🚀

Мы запускаем Партнёрскую программу Morpheus и ищем партнёров, которые могут помочь представить Morpheus большему числу клиентов, особенно тем, у кого есть потребности в кастомных AI-агентах или автоматизации.

Партнёры помогают связывать нас с потенциальными пользователями и проектами, а команда Morpheus предоставляет полную техническую поддержку, кастомизацию и интеграцию.
...
Morpheus выпускается под [лицензией MIT](LICENSE). Вы можете свободно использовать, модифицировать и распространять это программное обеспечение.
