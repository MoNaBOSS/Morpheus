...
Morpheus 现在以 **Signal OS** 形态启动：这是一个冷静、以执行为中心的智能操作界面，
拥有原创信号标识、电影化激活流程、紧凑产品导航，以及 **Today / Mission / Context**
指挥中心。Chat 仍然可用，但不再定义首页体验。
...
**询问 / 自动 / 执行**在语音、快捷命令、指挥中心和 Chat 中是同一个控制。默认的自动模式由主进程把问题路由到 OpenClaw，把明确的结果请求路由到同一 Objective Core；不清楚的指令只请求一次聚焦说明。指挥中心与快捷命令共享逻辑工作区和 Agent Profile 上下文，目标解析与执行权限始终由主进程掌控。
