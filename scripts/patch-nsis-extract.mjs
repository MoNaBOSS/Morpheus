...
const PATCHED_EXTRACT_MACRO = [
  ...
  '    IntOp $R9 $R9 + 1',
  '    DetailPrint "Extracting Morpheus application files (attempt $R9, please wait)..."',
  '    SetOutPath $INSTDIR',
  '    ClearErrors',
  '    Nsis7z::Extract "${FILE}"',
  ...
  '    ${endIf}',
  '    DetailPrint "Failed to extract Morpheus files after multiple attempts."',
  '    ${if} $clawxRollbackDir != ""',
...
export function patchNsisExtractTemplate(targetPath = EXTRACT_APP_PACKAGE_NSH) {
  ...
  if (hasStaleExtractPatch(original)) {
    console.warn('[patch-nsis-extract] Stale extract patch detected; replacing with Morpheus fail-closed patch.');
