...
# Post-installation script for Morpheus on Linux
...
# Create symbolic link for Morpheus app binary
if [ -x /opt/Morpheus/morpheus ]; then
    ln -sf /opt/Morpheus/morpheus /usr/local/bin/morpheus 2>/dev/null || true
fi

# Create symbolic link for openclaw CLI
OPENCLAW_WRAPPER="/opt/Morpheus/resources/cli/openclaw"
...
if ! { [[ -L /proc/self/ns/user ]] && unshare --user true; }; then
    ...
    chmod 4755 '/opt/Morpheus/chrome-sandbox' || true
else
    chmod 0755 '/opt/Morpheus/chrome-sandbox' || true
fi
...
if apparmor_status --enabled > /dev/null 2>&1; then
    APPARMOR_PROFILE_SOURCE='/opt/Morpheus/resources/apparmor-profile'
...
echo "Morpheus has been installed successfully."
